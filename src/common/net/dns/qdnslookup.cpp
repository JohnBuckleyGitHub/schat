...
{
    qRegisterMetaType<QDnsLookupReply>("QDnsLookupReply");
}
/*!
...
void QDnsLookup::lookup()
{
    ...
    d->isFinished = false;
    d->reply = QDnsLookupReply();
    d->runnable = new QDnsLookupRunnable(d->type, QDnsLookupRunnable::toAce(d->name));
...
QByteArray QDnsLookupRunnable::toAce(const QString &domain)
{
  QByteArray out = QUrl::toAce(domain);
  if (out.isEmpty())
    out = domain.toLatin1();
...
QString QDnsLookupRunnable::fromAce(const QByteArray &domain)
{
  QString out = QUrl::fromAce(domain);
  if (out.isEmpty())
    out = domain;
