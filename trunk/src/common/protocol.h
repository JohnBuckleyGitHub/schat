 ...
 */
static const int GreetingTimeout                    = 10;


/*!
 * Минимальный интервал для отправки пинг-пакета сервером.
 * Для нового протокола это время отсчитывается с момента
 * получения последнего пакета от клиента, для старого с
 * момента последнего понг-пакета.
 * Полное время обнаружения сетевой проблемы может достигать:
 * PingInterval + PongTimeout.
 */
static const int PingInterval                       = 18;


/*!
 * Время ожидание сервером ответа на пинг-пакет, в случае
 * не получения ответа за данное время соединение будет
 * разорвано.
 */
static const int PongTimeout                        = 5;
